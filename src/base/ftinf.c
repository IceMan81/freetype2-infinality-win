...
void ftinf_env(){
    const char *s;
    if( ftinf!=NULL ) return;   /* already called */
    ...
    s=getenv( "INFINALITY_FT_LCD_FILTER" );
    if( s==NULL )
        s=getenv( "INFINALITY_FT_FILTER_PARAMS" );
    if( s ) {
        int *f=_env.lcd_filter;
        if( sscanf( s, "%d %d %d %d %d", f+1, f+2, f+3, f+4, f+5 )==5 )
            f[0]=on;
        else
            f[0]=off;           /* FIXME: put a warning? */
    }
    s=getenv( "INFINALITY_FT_LCD_FILTER_LIGHT" );
    if( s ) {
        int *f=_env.lcd_filter_light;
        if( sscanf( s, "%d %d %d %d %d", f+1, f+2, f+3, f+4, f+5 )==5 )
            f[0]=on;
        else
            f[0]=off;           /* FIXME: put a warning? */
