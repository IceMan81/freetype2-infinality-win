...
#define STEM_START_RULES_SIZE 3
                  ...
                  Stem_Data*      known_stem_values )
  {
    ...
    {
      if ( family                                    &&
           ( _stricmp( SNAPPING_STEM_WIDTHS_Rules[i].family,
                         family ) == 0 ) )
      {
        j = 0;
    ...
    {
      if ( family                                    &&
           ( _stricmp( SNAPPING_STEM_SCALING_Rules[i].family,
                         family ) == 0 ) )
      {
    ...
    {
      if ( family                                                    &&
           ( _stricmp( SNAPPING_M_Rules[i].family, family ) == 0 ) )
      {
    ...
    {
      if ( family                                                        &&
           ( _stricmp( SNAPPING_STEM_TRANSLATING_ONLY_Rules[i].family,
                         family ) == 0 )                                 )
      {
    ...
    {
      if ( family                                                   &&
           ( _stricmp( SNAPPING_STEM_TRANSLATING_Rules[i].family,
                         family ) == 0 )                            )
      {
    ...
    {
      if ( family                                    &&
           ( _stricmp( ALWAYS_USE_100_Rules[i].family, family ) == 0 ) )
      {
    ...
    {
      if ( family                                                   &&
           ( _stricmp( SNAPPING_SYNTHESIZE_STEMS_Rules[i].family,
                         family ) == 0 )                            )
      {
    ...
    {
      if ( family                                                 &&
           ( _stricmp( SNAPPING_EDGE_DETECTION_Rules[i].family,
                         family ) == 0 )                          )
      {
    ...
    {
      if ( family                                                        &&
           ( _stricmp( SNAPPING_NO_BEARING_CORRECTION_Rules[i].family,
                         family ) == 0 )                                 )
      {
    ...
    {
      if ( family                                    &&
           ( _stricmp( BRIGHTNESS_Rules[i].family, family ) == 0 ) )
      {
    ...
    {
      if ( family                                    &&
           ( _stricmp( CONTRAST_Rules[i].family, family ) == 0 ) )
      {
    ...
    {
      if ( family                                    &&
           ( _stricmp( STEM_SPACING_Rules[i].family, family ) == 0 ) )
      {
        j = 0;
    ...
    {
      if ( family                                    &&
           ( _stricmp( STEM_START_Rules[i].family, family ) == 0 ) )
      {
        j = 0;
                  ...
                  )
  {
    ...
    *scale_value = 1.0;
    ...
    if ( !checked_use_known_settings_on_selected_fonts_env )
    {
      ...
      {
        if ( _stricmp( use_known_settings_on_selected_fonts_env,
                         "default" ) != 0 )
        {
          if ( _stricmp( use_known_settings_on_selected_fonts_env,
                           "true") == 0 )
            use_known_settings_on_selected_fonts = TRUE;
          else if ( _stricmp( use_known_settings_on_selected_fonts_env,
                                "1") == 0 )
            use_known_settings_on_selected_fonts = TRUE;
          else if ( _stricmp( use_known_settings_on_selected_fonts_env,
                                "on") == 0 )
    ...
    {
      if ( _stricmp( slot->face->style_name, "Italic" )            ||
           _stricmp( slot->face->style_name, "Oblique" )           ||
           _stricmp( slot->face->style_name, "Script" )            ||
           _stricmp( slot->face->style_name, "Handwriting" )       ||
           _stricmp( slot->face->style_name, "Bold" )              ||
           _stricmp( slot->face->style_name, "Black" )             ||
           ( ( _stricmp( slot->face->style_name, "Extra Thin" )  ||
               _stricmp( slot->face->style_name, "Extra Light" ) ) &&
             ppem < 10 )                                             ||
           ( _stricmp( slot->face->style_name, "Thin" )
               && ppem < 10 )                                        ||
           ( _stricmp( slot->face->style_name, "Light" )
               && ppem < 10 )                                        ||
           ( _stricmp( slot->face->style_name, "Narrow" )
               && ppem < 15 )                                        ||
           ( _stricmp( slot->face->style_name, "Condensed" )
               && ppem < 20 )                                        )
            return;
    }
    ...
    {
      if ( _stricmp( slot->face->family_name, "Italic" )            ||
           _stricmp( slot->face->family_name, "Oblique" )           ||
           _stricmp( slot->face->family_name, "Script" )            ||
           _stricmp( slot->face->family_name, "Handwriting" )       ||
           _stricmp( slot->face->family_name, "Bold" )              ||
           _stricmp( slot->face->family_name, "Black" )             ||
           ( ( _stricmp( slot->face->family_name, "Extra Thin" )  ||
               _stricmp( slot->face->family_name, "Extra Light" ) ) &&
             ppem < 10 )                                              ||
           ( _stricmp( slot->face->family_name, "Thin" )
               && ppem < 10 )                                         ||
           ( _stricmp( slot->face->family_name, "Light" )
               && ppem < 10 )                                         ||
           ( _stricmp( slot->face->family_name, "Narrow" )
               && ppem < 15 )                                         ||
           ( _stricmp( slot->face->family_name, "Condensed" )
               && ppem < 20 )                                         )
            return;
    }
    ...
    if ( slot->face->family_name )
    {
      if ( _stricmp(slot->face->family_name, "Courier" ) ||
           _stricmp(slot->face->family_name, "Serif" )   ||
           _stricmp(slot->face->family_name, "Times" )   )
        has_serifs = TRUE;
    }
	...
	stem_centers        = (FT_Long*) malloc ( (256) * sizeof ( FT_UInt ) );
    /* Initialize */
...
#define transformed_point( point ) point * *scale_value + *translate_value * 12
                            ...
                            FT_Render_Mode    required_mode )
  {
    ...
    if ( checked_use_various_tweaks_env == 0 )
    {
      ...
      {
        if ( _stricmp( use_various_tweaks_env, "default" ) != 0 )
        {
          if ( _stricmp( use_various_tweaks_env, "true") == 0)
            use_various_tweaks = TRUE;
          else if ( _stricmp( use_various_tweaks_env, "1") == 0)
            use_various_tweaks = TRUE;
          else if ( _stricmp( use_various_tweaks_env, "on") == 0)
            use_various_tweaks = TRUE;
          else if ( _stricmp( use_various_tweaks_env, "yes") == 0)
            use_various_tweaks = TRUE;
        }
      }
    ...
    if ( checked_gamma_correction_value == 0 )
    {
        ...
        if ( gamma_correction_value_env != NULL )
      {
        ...
        if ( _stricmp( gamma_correction_value_env, "default" ) != 0 )
        {
...
RERENDER:
    ...
    outline = &slot->outline;
      ...
      /*if ( !autohinted
        ...
        && slot->face->style_name
        && ( _stricmp(slot->face->family_name, "Courier New" )
          && ( _stricmp(slot->face->style_name, "Regular" )
            || _stricmp(slot->face->style_name, "Italic" ) ) ) )
      ...
      if ( slot->face )
      {
        if ( !autohinted                                              &&
            ...
            slot->face->family_name                                  &&
            slot->face->style_name                                   &&
            _stricmp( slot->face->family_name, "Times New Roman" ) &&
            _stricmp( slot->face->style_name, "Italic" )           )
        ...
        if ( use_known_settings_on_selected_fonts              &&
            ...
            slot->face->family_name                           &&
            slot->face->style_name                            &&
            _stricmp(slot->face->family_name, "FreeSerif" ) &&
            _stricmp(slot->face->style_name, "Italic" )     )
      ...
      if ( mode == FT_RENDER_MODE_LCD )
      {
        ...
        if ( autohinted )
        {
               ...
               (float)get_contrast( slot->face->family_name, ppem ) / 300.0 ) )
          {
            ...
            if ( slot->face->family_name                      &&
                 _stricmp(slot->face->family_name, "Mono" ) )
              is_fixed_name = TRUE;
